...
static block_t *ParseNALBlock( decoder_t *p_dec, bool *pb_ts_used, block_t *p_frag )
{
             ...
             ( i_nal_type >= 13 && i_nal_type <= 18 ) )
    {
        ...
        if( i_nal_type == NAL_SEI && ( p_sys->b_sps && p_sys->b_pps ) )
        {
...
static block_t *OutputPicture( decoder_t *p_dec )
{
    ...
    if( p_sys->b_header &&
            p_sys->slice.i_nal_type >= NAL_SLICE &&
            p_sys->slice.i_nal_type <= NAL_SLICE_IDR)
    {
        if ( p_sys->i_valid < REQUIRED_SEI_POINTS && p_sys->i_recovery_frames > -1 )
        {
            if( p_sys->slice.i_frame_num != p_sys->i_recovery_frames ||
                    p_sys->slice.i_frame_type != BLOCK_FLAG_TYPE_I)
                p_sys->b_valid_recovery_point = true;

            if( p_sys->i_recovery_frame < 0 ||
                    ((p_sys->i_recovery_frame - p_sys->slice.i_frame_num) &
                     ((1 << p_sys->i_log2_max_frame_num) - 1)) > p_sys->i_recovery_frames ) {
                p_sys->i_recovery_frame = (p_sys->slice.i_frame_num + p_sys->i_recovery_frames) &
                                            ((1 << p_sys->i_log2_max_frame_num) - 1);
                p_sys->i_recovery_frames = -1;

                if( !p_sys->b_valid_recovery_point )
                    p_sys->i_recovery_frame = p_sys->slice.i_frame_num;
            }
        }

        if( p_sys->slice.i_nal_type == NAL_SLICE_IDR ||
            p_sys->i_recovery_frame == p_sys->slice.i_frame_num ) {
            p_sys->i_recovery_frame = -1;
            p_sys->i_valid++;
        }
